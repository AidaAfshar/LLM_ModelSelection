...
class CorralHyperparam:
    ...
    def update_distribution(self, arm_idx, reward, more_info = dict([])):
      self.num_plays[arm_idx] += 1
      self.cumulative_rewards[arm_idx] += reward
      self.mean_rewards[arm_idx] = self.cumulative_rewards[arm_idx]*1.0/self.num_plays[arm_idx]
...
class BalancingHyperparamDoublingDataDriven:
    def __init__(self, m, dmin, delta =0.01, 
        ...
        self.missp_test_counter = [0 for _ in range(m)]
    ...
    def sample_base_index(self):
        if self.classic:
            potentials = np.array([float(v) for v in self.balancing_potentials])
            return int(np.argmin(potentials))
            # return np.argmin(self.balancing_potentials)
        else:
            if sum([np.isnan(x) for x in self.base_probas]) > 0:
    ...
    def update_distribution(self, algo_idx, reward, more_info = dict([])):
        ...
        if self.empirical:
            ...
            if empirical_regret_estimator > self.putative_bounds_multipliers[algo_idx]*np.sqrt(self.num_plays[algo_idx]):
                self.missp_test_counter[algo_idx] += 1
        ...
        if not self.empirical:
            print("Mis-Specification Test", self.missp_test_counter)
...
class BD3RB:
    def __init__(self, m, r_star, dmin, delta =0.01, 
        c = 1, classic = True, empirical= False):
        self.empirical = empirical
        self.minimum_putative = .0001
        self.maximum_putative = 10000
    ...
    def sample_base_index(self):
        if self.classic:
            ...
            print("sampled base index: ", int(np.argmin(potentials)))
            return int(np.argmin(potentials))
            # return np.argmin(self.balancing_potentials)
        else:
            if sum([np.isnan(x) for x in self.base_probas]) > 0:
    ...
    def update_distribution(self, algo_idx, reward, more_info = dict([])):
        ...
        U_i_t_upper_bound = (self.cumulative_rewards[algo_idx] + hoeffding_bonuses[algo_idx])*1.0/np.sqrt(self.num_plays[algo_idx])
        # empirical_regret_estimator = self.num_plays[algo_idx]*( max(U_t_lower_bounds) - U_i_t_upper_bound )
        empirical_regret_estimator = self.num_plays[algo_idx]*( self.r_star - U_i_t_upper_bound )
...
class PerfectBalancing:
    def __init__(self, m, r_star, mode="pessimistic", delta =0.01, c = 1, classic=True):
        self.m = m
        self.r_star = r_star 
        self.balancing_potentials = [np.sqrt(0) for _ in range(m)]
        self.c = c 
        self.mode = mode
    ...
    def update_distribution(self, algo_idx, reward, more_info = dict([])):
        ...
        hoeffding_bonuses = [self.c*np.sqrt(self.num_plays[i]*np.log((self.num_plays[i]+1)*1.0/self.delta)) for i in range(self.m)]
        pseudo_regret = self.num_plays[algo_idx]*self.r_star - self.cumulative_rewards[algo_idx] 
        if self.mode=="pessimistic":
            self.balancing_potentials[algo_idx] = max(pseudo_regret - hoeffding_bonuses[algo_idx], self.balancing_potentials[algo_idx])
        elif self.mode=="greedy":
            self.balancing_potentials[algo_idx] = max(pseudo_regret, self.balancing_potentials[algo_idx])
        elif self.mode=="optimistic":
            self.balancing_potentials[algo_idx] = max(pseudo_regret + hoeffding_bonuses[algo_idx], self.balancing_potentials[algo_idx])


        # hoeffding_bonuses = [self.c*np.sqrt(self.num_plays[i]*np.log((self.num_plays[i]+1)*1.0/self.delta)) for i in range(self.m)]
        # regrets = [self.num_plays[i]*self.r_star - self.cumulative_rewards[i] for i in range(self.m)]
        # if mode=="pessimistic":
        #     [self.balancing_potentials[i] = regrets[i] - hoeffding_bonuses[i] for i in range(self.m)]
        # elif mode=="greedy":
        #     [self.balancing_potentials[i] = regrets[i] for i in range(self.m)]
        ...
        print("Cumulative rewards ", self.cumulative_rewards)
        print("Num plays ", self.num_plays)
        print("Balancing Potentials", self.balancing_potentials)
        print("Mean rewards ", self.mean_rewards)
        print("Balancing probabilities ",self.base_probas)
...
def get_modsel_manager(modselalgo, num_parameters, num_timesteps , parameters = []):
    ...
    if modselalgo == "CorralLow":
        modsel_manager = CorralHyperparam(num_parameters,  eta = .1/np.sqrt(num_timesteps), T = num_timesteps) 
    elif modselalgo == "Corral":
        modsel_manager = CorralHyperparam(num_parameters, eta = 1/np.sqrt(num_timesteps), T = num_timesteps) 
    elif modselalgo == "CorralHigh":
        modsel_manager = CorralHyperparam(num_parameters,  eta = 10/np.sqrt(num_timesteps), T = num_timesteps) 

    elif modselalgo == "CorralSuperHigh":
        modsel_manager = CorralHyperparam(num_parameters,  eta = 50/np.sqrt(num_timesteps), T = num_timesteps)  
